...
- [ ] **htdemucs peaks at 1721–1881 MB against the 1536 MB budget** — over. Two levers ruled out by measurement, not argument: eviction cannot fix it (working set, not retention) and **thread count does not move it** (`BenchTests.demucsThreadSweep`: 1838 MB at the *minimum* 1 thread, spread ~3.5 % and non-monotonic, while 1 thread costs 45 % more wall time). Flatness across thread counts is itself evidence the arena is the driver. Remaining lever is `DisableCpuMemArena`/`DisableMemPattern` via a ~20-line ObjC++ shim over the private `CXXAPIOrtSessionOptions` category — access path verified in `m7-perf-results.md` §3.2, deferred for pbxproj + private-API reasons stated there. Held as a `withKnownIssue` in `BenchTests.demucsFootprint`
